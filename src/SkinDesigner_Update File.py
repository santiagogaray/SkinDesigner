...
Provided by SkinDesigner 0.0.65
    ...
    Args:
        _update: Set to "True" if you want this component to search through the current Grasshopper file and update SkinDesigner tools components.
    Returns:
        readMe: ...
"""
...
def isSkinDesignerTools(component):
    """Return True if a component is part of skinDesigner."""
    if component.Category == "SkinDesigner" and not component.Name == "SkinDesigner_Update SkinDesigner":
    ...
    for obj in document.Objects:
        ...
        if type(obj) == Grasshopper.Kernel.Special.GH_Cluster:
            ...
            for clusterObj in  clusterDoc.Objects:
                if type(clusterObj) == type(ghenv.Component)and isSkinDesignerTools(clusterObj):
                    if clusterObj.Locked:
                        continue
                    components.append(clusterObj)
        
        elif type(obj) == type(ghenv.Component)and isSkinDesignerTools(obj):
            if obj.Locked:
                continue
