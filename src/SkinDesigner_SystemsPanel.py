...
ghenv.Component.Name = "SkinDesigner_SystemsPanel"
ghenv.Component.NickName = 'SystemsPanel'
...
try:
    ...
    SGLibSystemsPanelController = sc.sticky["SGLib_SystemsPanelController"]
except:
...
def PanelCreate():
    ...
    if tmpWindowSystem : 
        try:
            Window = dict(width=tmpWindowSystem[0], height=tmpWindowSystem[1], fromLeft=tmpWindowSystem[2], \
                fromBottom=tmpWindowSystem[3], recess=tmpWindowSystem[4])
            windowDef = "AddWindow(width=Window['width'], height=Window['height'], fromLeft=Window['fromLeft'],"+\
                "fromBottom=Window['fromBottom'], recess=Window['recess'], conformSurface="+str(tmpWindowSystem[5])+")"
        except:  
    ...
    if tmpGeometryData and len(tmpGeometryData)==8 and tmpGeometryData[0]:
        try:
            ...
            raise
            warningData.append("Invalid 'customSystem' input - data discarded")
    ...
    # ---------Add Panel Controller -----------
    systemsPanelController = SGLibSystemsPanelController()
    myBasePanel.SetPanelProperty("CustomGeoController", systemsPanelController)
