...
To remove panels from the construction, zoom into the component and hit the lowest "-" sign that shows up on the input side.
    ...
    Returns:
        panelBay: A Panel list containing all the Panel objects in the order indicated  in the pattern. 
    ...
    if pattern:
        ...
        for i in range(len(Letters_List)):
            if Letters_List[i] in panel_List :
                panelBay.append(panel_List[Letters_List[i]])
    if pattern == None or pattern == [] :
        panelBay = panel_List.values()
        panelBay.reverse()
