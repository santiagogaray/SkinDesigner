...
Use this component to display a panel or panel bay in the scene and/or generate grasshopper breps of the panel elements.
    ...
    Args:
        _panel: Panel object or Panel_Bay (list of Panels) generated by Panel components.
...
for i, panel in enumerate(_panel): 
    if not isinstance(panel, SGLibPanel): 
        warningData.append( "Invalid panel #" + str(i+1) + " in PanelBay input"); _panel[i] = None
try: 
    while True: _panel.remove(None)
except: pass
if _panel == []: warningData.append("'_panel' input data is missing") 
locP = None
...
if _panel <> []and _panel[0] and locP:

    for i in range(len(_panel)):
        arrBoxPoints = [[offsetX, offsetY, offsetZ],[offsetX + _panel[i].GetPanelProperty("PanelWidth"), offsetY, offsetZ],\
            [offsetX, offsetY, offsetZ+_panel[i].GetPanelProperty("PanelHeight")],\
            [offsetX + _panel[i].GetPanelProperty("PanelWidth"), offsetY, offsetZ+_panel[i].GetPanelProperty("PanelHeight")]]
        try:
            Mockup_Bay.append(SGLibPanel())
            Mockup_Bay[i].Copy(_panel[i])
